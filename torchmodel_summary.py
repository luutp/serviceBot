...
def get_summary(model, input_size, batch_size=-1, device="cuda", display = False):
    ...
    if display is True:
        print_summary(summary, input_size)
    return summary
        
def print_summary(summary, input_size, batch_size = -1):
    print("----------------------------------------------------------------")
...
def logging_model_summary(model, input_size, batch_size = -1):
    summary = get_summary(model, input_size, batch_size, device="cuda")
    logging.info("----------------------------------------------------------------")
    line_new = "{:>20}  {:>25} {:>15}".format("Layer (type)", "Output Shape", "Param #")
